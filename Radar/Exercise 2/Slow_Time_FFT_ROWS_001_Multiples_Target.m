...
for pulse = 1:numberOfPulses
    ...
    for n = 1:1:length(targetRadialDistance)
        ...
        receivedPulseTau = exp(1j*2*pi*(carrierFrequency+dopplerEffect(n)).*(tauTimeAxis-tau)).*...
            exp(1j*pi*chirpSlope*(1+dopplerRatio(n)).*(tauTimeAxis-tau).^2);

        receivedPulseTemp(n,:) = [exp(1j*2*pi*(carrierFrequency+dopplerEffect(n)).*(chirpTimeAxis-tau)).*...
            exp(1j*pi*chirpSlope*(1+dopplerRatio(n)).*(chirpTimeAxis-tau).^2) ...
            zeros(1, round((pulseDuration-chirpDuration)*samplingRate))];
