...
radialVelocity2 = kmph2mps(150);  % m/s
...
dopplerEffect2 = carrierFrequency*2*radialVelocity2/speedOfLight;
dopplerRatio = dopplerEffect/carrierFrequency;
dopplerRatio2 = dopplerEffect2/carrierFrequency;
...
radialDistanceCorrected2 = zeros(1, numberOfPulses);
for pulse = 1:numberOfPulses
    ...
    %Target 2
    radialDistanceCorrected2(pulse) = targetRadialDistance2 + ((pulse-1)* ...
        radialVelocity2*pulseDuration);
    receiverDelaySample2 = round(2*samplingRate*radialDistanceCorrected2(pulse)/ ...
        speedOfLight);
...
%         receiverDelaySample)];
    ...
    %Target 2
    tau2 = (2*radialDistanceCorrected2(pulse))/speedOfLight;
...
%         receiverDelaySample)];
    ...
    receivedPulse1 = [exp(1j*2*pi*(carrierFrequency.*(chirpTimeAxis-tau)-dopplerEffect.*dopplerRatio.*chirpTimeAxis)).*...
        exp(1j*pi*chirpSlope.*(chirpTimeAxis-tau-(dopplerRatio/2).*chirpTimeAxis).^2) ...
        zeros(1, round((pulseDuration-chirpDuration)*samplingRate))];
    
    receivedPulse1 = [zeros(1, receiverDelaySample) ...
        receivedPulse1(receiverDelaySample:(round(chirpDuration*samplingRate))) ...
        receivedPulseTau ...
        zeros(1, round((pulseDuration-chirpDuration)*samplingRate)-...
        receiverDelaySample)];
    
    % Target 2
    tauTimeAxis2 = linspace(chirpDuration,chirpDuration+tau2,tau2*samplingRate);
    receivedPulseTau2 = exp(1j*2*pi*(carrierFrequency.*(tauTimeAxis2-tau2)-dopplerEffect2.*dopplerRatio2.*tauTimeAxis2)).*...
        exp(1j*pi*chirpSlope.*(tauTimeAxis2-tau2-(dopplerRatio2/2).*tauTimeAxis2).^2);

    receivedPulse2 = [exp(1j*2*pi*(carrierFrequency.*(chirpTimeAxis-tau2)-dopplerEffect2.*dopplerRatio2.*chirpTimeAxis)).*...
        exp(1j*pi*chirpSlope.*(chirpTimeAxis-tau2-(dopplerRatio2/2).*chirpTimeAxis).^2) ...
        zeros(1, round((pulseDuration-chirpDuration)*samplingRate))];
    
    receivedPulse2 = [zeros(1, receiverDelaySample2) ...
        receivedPulse2(receiverDelaySample2:(round(chirpDuration*samplingRate))) ...
        receivedPulseTau2 ...
        zeros(1, round((pulseDuration-chirpDuration)*samplingRate)-...
        receiverDelaySample2)];
    
    if(length(receivedPulse1)==length(receivedPulse2))
        receivedPulse = receivedPulse1+receivedPulse2;
    elseif(length(receivedPulse1)>length(receivedPulse2))
        receivedPulse = receivedPulse1(1:(end-1))+receivedPulse2;
    else
        receivedPulse = receivedPulse1+receivedPulse2(1:(end-1));
    end
